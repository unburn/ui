...
export const parseColor = (color: string): ColorMetrics | null => {
  if (!color || typeof color !== 'string') return null;

  let r = 0, g = 0, b = 0, a = 1;
  const normalized = color.trim().toLowerCase();

  if (cssNamedColors[normalized]) {
    return parseColor(cssNamedColors[normalized]);
  }
  ...
  if (color.startsWith('#')) {
    ...
    const lightNames = ['white', 'yellow', 'cyan', 'lightgray', 'silver'];
    if (lightNames.includes(normalized)) {
      return { r: 255, g: 255, b: 255, a: 1, luminance: 1, isLight: true };
    }
  }
  ...
  return { r, g, b, a, luminance, isLight };
};
...
export const getAccentVariables = (color: string | undefined): React.CSSProperties => {
  if (!color) return {};
